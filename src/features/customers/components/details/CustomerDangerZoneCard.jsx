  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-2.5 px-1">
        ...
        <h3 className="text-[18px] font-extrabold tracking-tight text-[#d83f3f]">{ct("Administrative Danger Zone")}{" "}</h3>
      </div>
      ...
      <article className="flex flex-col justify-between gap-5 rounded-[16px] border border-[#fbcaca] bg-gradient-to-br from-white to-[#fffcfc] p-4 shadow-[0_4px_16px_rgba(216,63,63,0.02)] transition-all duration-300 hover:border-[#d83f3f]/30 hover:shadow-md sm:p-6">
        <div className="space-y-2">
          <h4 className="text-[16px] font-extrabold text-[#18120f]">{ct("Deactivate Customer Account")}{" "}</h4>
          <p className="text-[13px] leading-relaxed font-medium text-[#5a4d46]">{ct("Deactivation keeps historical order and billing records intact, but prevents")}{" "}{customerName}{ct("from using the platform until the account is restored manually.")}{" "}</p>
        </div>
        <button
          ...
          type="button"
        >
          {isSubmitting ? ct("Deactivating...") : ct("Deactivate Account")}
        </button>
      </article>
    </section>
  );
