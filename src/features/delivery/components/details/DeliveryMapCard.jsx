    ...
    <section className="overflow-hidden rounded-[18px] border border-[#ddd4cd] bg-white shadow-[0_10px_24px_rgba(55,31,13,0.05)]">
      <div className="flex flex-col gap-4 px-5 py-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-[21px] font-bold tracking-[-0.03em] text-[#18120f]">{dt("Coverage Map:")}{area.city}</h2>
            <p className="mt-1.5 text-[14px] leading-6 text-[#7a6d66]">{dt("Visualizing active delivery radiuses and vendor clusters across the selected area.")}</p>
          </div>

          <div className="flex items-center gap-1.5 whitespace-nowrap">
            <CoverageChip label={dt("Active")} />
            <CoverageChip label={dt("Limited")} />
            <CoverageChip label={dt("Inactive")} />
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-[16px] border border-[#e8ddd5] bg-[linear-gradient(180deg,#fffaf6_0%,#fff3ea_100%)] p-5">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-[16px] border border-[#eadfd6] bg-white px-4 py-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9b8f86]">{dt("Map Center")}</p>
              <p className="mt-2 text-[15px] font-semibold text-[#18120f]">
                {center.lat != null && center.lng != null ? `${center.lat}, ${center.lng}` : dt("Not configured")}
              </p>
              <p className="mt-1 text-[12px] text-[#7a6d66]">{dt("Zoom level:")}{area.map?.zoom ?? 10}</p>
            </div>

            <div className="rounded-[16px] border border-[#eadfd6] bg-white px-4 py-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9b8f86]">{dt("Coverage Shapes")}</p>
              <p className="mt-2 text-[28px] font-extrabold tracking-[-0.04em] text-[#18120f]">{polygons.length}</p>
              <p className="mt-1 text-[12px] text-[#7a6d66]">{polygonPoints}{dt("polygon points loaded")}</p>
            </div>

            <div className="rounded-[16px] border border-[#eadfd6] bg-white px-4 py-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9b8f86]">{dt("Coverage Markers")}</p>
              <p className="mt-2 text-[28px] font-extrabold tracking-[-0.04em] text-[#18120f]">{markers.length}</p>
              <p className="mt-1 text-[12px] text-[#7a6d66]">{dt("Active map reference points")}</p>
            </div>
          </div>

          <div className="mt-4 rounded-[16px] border border-[#eadfd6] bg-white px-4 py-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9b8f86]">{dt("Map Bounds")}</p>
          ...
          <div className="mt-4 rounded-[16px] border border-dashed border-[#eadfd6] bg-white/80 px-4 py-5">
            <div className="flex items-center gap-2 text-[#cf6e38]">
              <MapPinned size={16} />
              <p className="text-[13px] font-bold text-[#2b211b]">{dt("Map data connected")}</p>
            </div>
            <p className="mt-2 text-[13px] leading-6 text-[#786d66]">{dt("The backend is returning polygon and marker coordinates for this delivery area. A visual GIS renderer can be added on top of this payload without changing the GraphQL integration.")}</p>
            {markers.length ? (
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {polygons.slice(0, 6).map((polygon) => (
                  <span
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full border border-[#ead2c3] bg-white px-3 py-1.5 text-[12px] font-medium text-[#4f4036]"
                  >
                    <span>{polygon.label || dt("Polygon {{v0}}", { v0: polygon.id })}</span>
                  </span>
                ))}
              </div>
            ) : null}
