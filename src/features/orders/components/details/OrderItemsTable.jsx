  ...
  return (
    <article className="w-full overflow-hidden rounded-[14px] border border-[#ddd6cf] bg-white shadow-[0_6px_16px_rgba(53,34,20,0.05)]">
      <header className="border-b border-[#eee4dd] px-5 py-4">
        <h3 className="text-[16px] font-bold text-[#18120f]">{ot("Order Items")}</h3>
      </header>
        ...
        <table className="w-full min-w-[650px] border-collapse">
          <thead>
            <tr className="border-b border-[#eee4dd] bg-[#fcfbfa] text-left">
              <th className="px-5 py-3 text-[12px] font-bold text-[#9b8f86]">{ot("Item")}</th>
              <th className="px-5 py-3 text-center text-[12px] font-bold text-[#9b8f86]">{ot("Qty")}</th>
              <th className="px-5 py-3 text-right text-[12px] font-bold text-[#9b8f86]">{ot("Unit Price")}</th>
              <th className="px-5 py-3 text-right text-[12px] font-bold text-[#9b8f86]">{ot("Total")}</th>
            </tr>
          </thead>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr>
                <td className="px-5 py-8 text-center text-[14px] text-[#6f645d]" colSpan={4}>{ot("No line items available for this order.")}</td>
              </tr>
            ) : (
              items.map((item) => (
                  ...
                  <td className="px-5 py-3.5 align-middle">
                    <div className="flex items-start gap-3">
                      {item.imageUrl ? (
                        ...
                        <div className="flex h-11 w-11 items-center justify-center rounded-[6px] bg-[#f6eee8] text-[10px] font-bold text-[#6f655e]">{ot("ITEM")}</div>
                      )}

                      <div>
                        <p className="text-[14px] font-bold text-[#18120f]">{item.name}</p>
                        {onViewItemSource && item.menuId ? (
                          <button
                            ...
                            type="button"
                          >{ot("View Menu")}<ArrowUpRight size={12} />
                          </button>
                        ) : null}
                        {item.notes ? (
                        ...
                        {item.options?.length > 0 ? (
                          <p className="mt-0.5 text-[11px] text-[#8c8077]">{ot("Options:")}{item.options.join(", ")}
                          </p>
                        ) : null}
                        {item.addons.length > 0 ? (
                          <p className="mt-0.5 text-[11px] text-[#8c8077]">{ot("Add-ons:")}{" "}
                            {item.addons
                              .map(
                                (addon) =>
