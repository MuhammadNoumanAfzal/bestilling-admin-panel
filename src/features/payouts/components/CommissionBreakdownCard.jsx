  ...
  return (
    <section className="overflow-hidden rounded-[18px] border border-[#d8ccc2] bg-white shadow-[0_10px_22px_rgba(56,33,17,0.04)]">
      <div className="flex flex-col gap-3 border-b border-[#e9dfd8] bg-[linear-gradient(180deg,#fff8f2_0%,#fffdfa_100%)] px-5 py-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-[18px] font-bold text-[#221914] sm:text-[20px]">{pt("Commission Breakdown")}</h2>
          <p className="mt-1.5 text-[14px] leading-6 text-[#7b6f68]">{pt("Review the default platform split and high-performing vendor contribution by region.")}</p>
        </div>
        ...
        <div className="rounded-[16px] border border-[#f2d7c7] bg-white px-4 py-2.5 shadow-[0_8px_20px_rgba(59,33,18,0.05)]">
          <p className="text-[13px] font-bold text-[#cf6e38]">
            {pt(globalCommissionLabel || "Platform Default Commission")}
          </p>
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <div className="border-b border-[#e9dfd8] px-5 py-3.5 lg:border-b-0 lg:border-r">
          <p className="text-[13px] font-bold uppercase tracking-[0.08em] text-[#8b7d75]">{pt("Region")}</p>
          <div className="mt-2.5 space-y-2">
            {hasRegions ? (
              regions.map((region) => <RegionRow key={region.id} label={region.label} value={region.value} />)
            ) : (
              <p className="rounded-[14px] border border-dashed border-[#eadfd6] px-4 py-6 text-[14px] text-[#8b7d75]">{pt("No area commission overrides are configured yet.")}</p>
            )}
          </div>
        </div>

        <div className="px-5 py-3.5">
          <p className="text-[13px] font-bold uppercase tracking-[0.08em] text-[#8b7d75]">{pt("Vendors")}</p>
          <div className="mt-2.5 grid gap-2 sm:grid-cols-2">
            {hasVendors ? (
              vendors.map((vendor) => <VendorShareRow key={vendor.id} {...vendor} />)
            ) : (
              <p className="rounded-[14px] border border-dashed border-[#eadfd6] px-4 py-6 text-[14px] text-[#8b7d75] sm:col-span-2">{pt("No vendor commission overrides are configured yet.")}</p>
            )}
          </div>
        </div>
      </div>
