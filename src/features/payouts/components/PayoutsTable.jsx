...
export default function PayoutsTable({
  ...
  return (
    <div className="payout-table-container m-2 overflow-hidden rounded-[14px] border border-[#d9cdc4] bg-white shadow-[0_10px_22px_rgba(56,33,17,0.04)]">
      <div className="payout-view-switch flex flex-wrap gap-2 border-b border-[#eee4dd] p-3" role="group" aria-label={pt("Payment record view")}>
      ...
      <div className="w-full overflow-x-auto" role="region" aria-label={pt("Payments table")} tabIndex={0}>
        <table className={`payout-table payout-view-${view} w-full border-collapse`}>
          <thead className="border-b border-[#eee4dd] bg-[#fcfbfa]">
            <tr className="text-left">
              <th className="px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Order ID")}</th>
              <th className="px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Date")}</th>
              <th className="payout-order-column payout-customer-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Customer")}</th>
              <th className="payout-order-column payout-vendor-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Vendor")}</th>
              <th className="payout-order-column payout-customer-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Order Amount")}</th>
              <th className="payout-order-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Order Status")}</th>
              <th className="payout-customer-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Customer Payment Status")}</th>
              <th className="payout-vendor-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Platform Commission")}</th>
              <th className="payout-vendor-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Vendor Amount")}</th>
              <th className="payout-vendor-column px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{pt("Vendor Payout Status")}</th>
              <th className="px-4 py-4 text-right text-[13px] font-bold text-[#9b8f86]">{pt("Actions")}</th>
            </tr>
          </thead>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td className="px-4 py-10 text-center text-[15px] font-medium text-[#6f645d]" colSpan={11}>{pt("No payment records match the current filters.")}</td>
              </tr>
            ) : (
              rows.map((row) => {
                      ...
                      <div className="ml-auto flex w-[148px] flex-col items-stretch gap-1.5">
                        {canApproveReportedPayment ? (
                          <button
                            ...
                            type="button"
                          >
                            {activeActionKey === `${row.id}:approveInvoice` ? pt("Approving...") : pt("Approve Payment")}
                          </button>
                        ) : null}
                        {canMarkCustomerReceived ? (
                          <button
                            ...
                            type="button"
                          >
                            {activeActionKey === `${row.id}:markReceived` ? pt("Updating...") : pt("Mark Received")}
                          </button>
                        ) : null}
                        {canMarkVendorPaid ? (
                          <button
                        ...
                        {showVendorPaymentWaiting ? (
                          <span className="inline-flex min-h-[34px] w-full items-center justify-center rounded-[10px] border border-[#eadccd] bg-[#fbf7f3] px-2.5 text-center text-[10px] font-bold leading-4 text-[#9b8f86]">{pt("Awaiting Customer Payment")}</span>
                        ) : null}
                        <button
      ...
      <div className="flex flex-col gap-4 border-t border-[#eee4dd] px-4 py-4 text-[13px] text-[#6c6058] sm:flex-row sm:items-center sm:justify-between">
        <p>
          {pt("Showing {{start}} - {{end}} of {{total}} Orders", { start, end, total: totalItems })}
        </p>
