...
function InfoCell({ accent = false, label, value }) {
  ...
  return (
    <div className="rounded-[18px] border border-[#eee4dd] bg-[linear-gradient(180deg,#ffffff_0%,#fcfaf8_100%)] px-4 py-4 shadow-[0_8px_18px_rgba(55,31,13,0.04)]">
      <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#a29388]">{pt(label)}</p>
    ...
    <section className="overflow-hidden rounded-[24px] border border-[#ddd4cd] bg-white shadow-[0_14px_34px_rgba(55,31,13,0.06)]">
      <div className="flex flex-col gap-3 border-b border-[#eee5de] bg-[linear-gradient(180deg,#fff8f3_0%,#ffffff_100%)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 text-[#221914]">
          ...
          <h2 className="text-[18px] font-bold">{pt("Order Information")}</h2>
        </div>
      ...
      <div className="grid gap-4 px-5 py-5 sm:grid-cols-2 xl:grid-cols-3">
        <InfoCell label={pt("Invoice Number")} value={payout.invoiceNumber} />
        <InfoCell label={pt("Customer Name")} value={payout.customer.fullName} />
        <InfoCell accent label={pt("Vendor Name")} value={payout.vendor.name} />
        <InfoCell label={pt("Vendor Contact")} value={payout.vendor.contactName || pt("Not available")} />
        <InfoCell label={pt("Order Created")} value={payoutDate(payout.order.createdAtLabel)} />
        <InfoCell label={pt("Last Updated")} value={payoutDate(payout.updatedAtLabel)} />
        <InfoCell label={pt("Notes")} value={payout.notes || pt("No payment notes added.")} />
      </div>
    </section>
  );
}
