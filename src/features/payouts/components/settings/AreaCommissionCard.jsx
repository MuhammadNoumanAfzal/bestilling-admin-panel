    ...
    <article className="rounded-[16px] border border-[#eadfd6] bg-[linear-gradient(180deg,#fffdfa_0%,#fff8f2_100%)] p-4 shadow-[0_10px_20px_rgba(56,33,17,0.05)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[12px] text-[#7d7068]">{cmt("{{count}} active vendors", { count: row.activeVendors })}</p>
        </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        ...
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9b8f86]">{cmt("Order Share")}</p>
    ...
    <section className="overflow-hidden rounded-[16px] border border-[#d8ccc2] bg-white shadow-[0_10px_22px_rgba(56,33,17,0.04)]">
      <div className="flex items-center justify-between gap-3 px-4 py-4">
        <div className="flex items-center gap-2">
          <h2 className="text-[20px] font-bold text-[#221914] sm:text-[22px]">{cmt("Area Commission")}</h2>
      ...
      <div className="flex flex-col gap-3 px-4 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="relative w-full sm:max-w-[240px]">
          <input
            ...
            placeholder={cmt("Search area or commission...")}
            type="search"
            value={searchTerm}
          />
        ...
        >
          ...
          <span>{cmt("Add Area Commission")}</span>
        </button>
      </div>

      <div className="space-y-3 border-t border-[#eee4dd] p-3 md:hidden">
        {filteredRows.length === 0 ? (
          <div className="rounded-[14px] border border-dashed border-[#e2d7cf] bg-[#fcfaf8] px-4 py-10 text-center text-[15px] font-medium text-[#6f645d]">{cmt("No area commissions match the current search.")}</div>
        ) : (
        ...
        <table className="w-full table-fixed border-collapse">
          <thead className="bg-[#fcfbfa]">
            <tr className="text-left">
              <th className="px-4 py-4 text-[13px] font-bold text-[#9b8f86]">{cmt("Area")}</th>
              <th className="px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{cmt("Commission Rate")}</th>
              <th className="px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{cmt("Active Vendors")}</th>
              <th className="px-3 py-4 text-[13px] font-bold text-[#9b8f86]">{cmt("Order Share")}</th>
          ...
          <tbody>
            {filteredRows.length === 0 ? (
              <tr className="border-t border-[#f1e9e2]">
                <td className="px-4 py-10 text-center text-[15px] font-medium text-[#6f645d]" colSpan={5}>{cmt("No area commissions match the current search.")}</td>
              </tr>
            ) : (
              filteredRows.map((row) => (
