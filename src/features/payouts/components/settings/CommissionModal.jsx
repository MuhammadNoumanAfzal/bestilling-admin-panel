...
function Field({ children, label }) {
  ...
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[12px] font-bold text-[#2f241d]">{cmt(label)}</span>
      {children}
    </label>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#211713]/50 px-4 py-4 backdrop-blur-[4px]">
      <div className="flex max-h-[84vh] w-full max-w-[560px] flex-col overflow-hidden rounded-[24px] border border-[#ecdccf] bg-[linear-gradient(180deg,#fffdfa_0%,#fff7f1_100%)] shadow-[0_30px_80px_rgba(28,18,12,0.22)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#f1e2d8] px-5 py-4">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#cf6e38]">{cmt("Commission Setup")}</p>
            <h2 className="mt-1 text-[24px] font-bold tracking-[-0.03em] text-[#18120f]">{cmt(title)}</h2>
          </div>
              ...
              <div key={field.key} className={field.fullWidth ? "sm:col-span-2" : ""}>
                <Field label={field.label}>
                  {field.type === "select" ? (
                    ...
                    <div className="space-y-2">
                      <input
                        ...
                        placeholder={cmt(field.searchPlaceholder) || cmt("Search {{field}}", { field: cmt(field.label) })}
                        type="search"
                      ...
                      <select
                        ...
                        value={field.value}
                      >
                        <option value="">{cmt(field.placeholder) || cmt("Select {{field}}", { field: cmt(field.label) })}</option>
                        {field.options.map((option) => (
                          <option key={option.value} value={option.value}>
                            {option.label}
                          </option>
                      ...
                      <p className="min-h-[18px] text-[11px] text-[#8e7f75]">
                        {field.isLoadingOptions
                          ? cmt("Loading options...")
                          : cmt(field.helperText) ||
                            (field.options.length === 0 ? cmt("No matching options found.") : "")}
                      </p>
                    </div>
                  ) : field.type === "textarea" ? (
                    <textarea
                      ...
                      disabled={field.disabled || isSubmitting}
                      onChange={(event) => onChange(field.key, event.target.value)}
                      placeholder={cmt(field.placeholder)}
                      rows={field.rows || 4}
                      value={field.value}
                    />
                  ) : (
                  ...
                  {field.type !== "search-select" && field.helperText ? (
                    <p className="text-[11px] leading-5 text-[#8e7f75]">{cmt(field.helperText)}</p>
                  ) : null}
                </Field>
              </div>
            ))}
        ...
        <div className="flex flex-wrap items-center justify-end gap-2.5 border-t border-[#f1e2d8] px-5 py-4">
          ...
          >
            <Save size={14} />
            <span>{isSubmitting ? cmt("Saving...") : cmt(submitLabel)}</span>
          </button>
        </div>
      </div>
    </div>
