  ...
  return (
    <section className="overflow-hidden rounded-[16px] border border-[#d8ccc2] bg-white shadow-[0_10px_22px_rgba(56,33,17,0.04)]">
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_240px]">
        <div className="border-b border-[#eee4dd] px-4 py-4 sm:px-5 sm:py-5 lg:border-b-0 lg:border-r">
          ...
          <h2 className="mt-3 text-[18px] font-bold text-[#221914] sm:text-[20px]">{cmt("Platform Default Commission")}</h2>
          <p className="mt-2 max-w-[72ch] text-[14px] leading-6 text-[#6f645d]">{cmt(settings.description)}</p>
        </div>

        <div className="flex flex-col justify-center gap-4 bg-[linear-gradient(180deg,#fffdfa_0%,#fff7f1_100%)] px-4 py-4 sm:px-5 sm:py-5">
          <div className="rounded-[18px] border border-[#efddd2] bg-white px-4 py-4 text-center shadow-[0_10px_22px_rgba(56,33,17,0.05)]">
            <p className="text-[32px] font-bold tracking-[-0.05em] text-[#1d1612] sm:text-[36px]">{settings.currentRate}</p>
            <p className="mt-1 text-[11px] font-bold uppercase tracking-[0.12em] text-[#8d8077]">{cmt("Current Rate")}</p>
          </div>
          ...
          <button
            ...
            onClick={onEdit}
            type="button"
          >{cmt("Edit Global Commission")}</button>
        </div>
      </div>
    </section>
  );
