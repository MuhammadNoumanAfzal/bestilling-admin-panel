    ...
    return (
      <ReportsSectionCard className="h-full p-6">
        <div className="space-y-3">
          <h2 className="text-[18px] font-extrabold tracking-[-0.04em] text-[#18120f] sm:text-[20px]">{rt("Category Performance")}</h2>
          <p className="text-[12px] font-medium text-[#8a7d74]">{rt("Market share by event type")}</p>
          <div className="rounded-[14px] border border-dashed border-[#e1d7d0] bg-[#fbf8f5] px-4 py-10 text-center text-[14px] font-medium text-[#7a6e67]">{rt("No category performance data is available for the selected period.")}</div>
        </div>
      </ReportsSectionCard>
    );
  }
      ...
      <div className="flex h-full flex-col gap-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <span className="inline-flex rounded-full border border-[#f0d9ca] bg-white/90 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.24em] text-[#d16936]">{rt("Revenue Mix")}</span>
            <h2 className="mt-3 text-[20px] font-black tracking-[-0.05em] text-[#18120f] sm:text-[22px]">{rt("Category Performance")}</h2>
            <p className="mt-1 text-[12px] font-medium text-[#8a7d74]">{rt("Market share by event type")}</p>
          </div>
          ...
          <div className="w-full rounded-[18px] border border-[#edd9ca] bg-white/80 px-4 py-3 shadow-[0_10px_26px_rgba(55,31,13,0.05)] lg:max-w-[220px]">
            <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#9e8879]">{rt("Top Segment")}</p>
            <p
              className="mt-1 break-words text-[15px] font-black tracking-[-0.03em]"
              style={{ color: primaryCategory?.color || "#d16936" }}
            >
              {rt(primaryCategory?.label || "N/A")}
            </p>
            <p className="mt-1 text-[12px] font-semibold text-[#5d5047]">{rt("{{value}}% of revenue mix", { value: primaryCategory?.value || 0 })}</p>
          </div>
        </div>
              ...
              >
                <div className="absolute inset-[28px] flex items-center justify-center rounded-full bg-[radial-gradient(circle_at_top,#fffefd_0%,#fff6ef_70%,#f6e5d9_100%)] shadow-[inset_0_2px_10px_rgba(255,255,255,0.85)] sm:inset-[30px] lg:inset-[34px]">
                  <div className="text-center">
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-[#9c8779]">{rt("Leader")}</p>
                    ...
                    <p className="mt-1 max-w-[72px] text-[10px] font-semibold leading-4 text-[#7c6f66] sm:max-w-[84px] sm:text-[11px]">
                      {rt(primaryCategory?.label || "No data")}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <p className="truncate text-[15px] font-bold text-[#211711]">{rt(category.label)}</p>
                      </div>
                    ...
                    <div className="shrink-0 sm:text-right">
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#9f897a]">
                        {isPrimary ? rt("Top") : rt("Rank {{rank}}", { rank: index + 1 })}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {secondaryCategories.length ? (
          <div className="rounded-[18px] border border-[#efdfd3] bg-white/65 px-4 py-3 text-[12px] leading-6 text-[#6d6057]">{rt("Secondary categories continue to contribute meaningful revenue spread, helping reduce over-reliance on a single event type.")}</div>
        ) : null}
      </div>
    </ReportsSectionCard>
  );
