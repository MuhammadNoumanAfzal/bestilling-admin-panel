...
function MetricTile({ label, value, note }) {
  ...
  return (
    <div className="rounded-[14px] border border-[#e9dfd8] bg-[#fbf9f7] px-3.5 py-3">
      <p className="text-[11px] font-semibold uppercase tracking-[0.03em] text-[#8b7f76]">{rt(label)}</p>
      <p className="mt-1.5 text-[28px] font-extrabold leading-none tracking-[-0.05em] text-[#1e1611]">
        {value}
      </p>
      <p className="mt-1.5 text-[11px] font-medium text-[#8b7f76]">{rt(note)}</p>
    </div>
  );
}
  ...
  return (
    <ReportsSectionCard className="h-full">
      <div className="space-y-4">
        <div>
          <h2 className="text-[20px] font-extrabold tracking-[-0.04em] text-[#18120f]">{rt("Customer Analytics")}</h2>
          <p className="text-[12px] font-medium text-[#8a7d74]">{rt("Acquisition and retention metrics")}</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {stats.length ? (
            ...
            <div className="rounded-[14px] border border-dashed border-[#e1d7d0] bg-[#fbf8f5] px-4 py-10 text-center text-[14px] font-medium text-[#7a6e67] sm:col-span-2">{rt("No customer analytics data is available for the selected period.")}</div>
          )}
        </div>

        <div className="rounded-[16px] border border-[#84b6ff] bg-[#edf5ff] px-4 py-3.5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="text-[12px] font-bold text-[#2570dd]">{rt("Customer Satisfaction Score")}</p>
          ...
          <p className="mt-3 text-[11px] font-medium text-[#4e7dc1]">{rt(satisfaction.note)}</p>
        </div>
      </div>
    </ReportsSectionCard>
  );
