  ...
  return (
    <div className="flex shrink-0 flex-col items-start gap-2">
      <button
        ...
        onClick={onClick}
        title={isDisabled ? st(disabledMessage) : st("Update avatar")}
        type="button"
      >
        {avatarUrl ? (
          <img alt={st("Administrator avatar")} className="h-full w-full object-cover" src={avatarUrl} />
        ) : initials ? (
      ...
      <span className="max-w-[140px] text-[10px] font-medium leading-4 text-[#9f948c]">
        {isUpdating ? st("Uploading...") : isDisabled ? disabledMessage : st("Update Avatar")}
      </span>
    </div>
  );
}
