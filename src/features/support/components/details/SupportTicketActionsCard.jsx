  ...
  return (
    <section className="rounded-[18px] border border-[#eadcd3] bg-white p-3.5 shadow-[0_8px_20px_rgba(55,31,13,0.05)]">
      <div className="mb-3 rounded-[14px] bg-[linear-gradient(135deg,#fff7f2_0%,#fff4ec_50%,#fffdfa_100%)] px-4 py-3.5">
        <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-[#9b8f86]">{st("Quick Actions")}</p>
        <p className="mt-1.5 text-[14px] leading-6 text-[#746861]">{st("Keep the ticket moving by updating ownership, priority, and status without leaving the thread.")}</p>
      </div>

      <div className="space-y-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#9b8f86]">{st("Current State")}</p>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#9b8f86]">{st("Priority")}</span>
          <select
        ...
        <div className="rounded-[14px] border border-[#efe2d8] bg-[#fcf8f5] px-3 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#9b8f86]">{st("Assignment")}</p>
          <p className="mt-2 text-[13px] font-semibold text-[#2f241d]">{assigneeName || st("Unassigned")}</p>
          <div className="mt-3 grid gap-2">
            {canAssignToMe ? (
              <ActionButton disabled={isAssigning} onClick={onAssignToMe}>
                {isAssigning ? <LoaderCircle className="animate-spin" size={14} /> : st("Assign To Me")}
              </ActionButton>
            ) : null}
            <ActionButton disabled={isAssigning || !assigneeName} onClick={onUnassign}>
              {isAssigning ? <LoaderCircle className="animate-spin" size={14} /> : st("Remove Assignment")}
            </ActionButton>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <ActionButton disabled={isUpdatingStatus || status === "IN_PROGRESS"} onClick={onSetInProgress} tone={status === "IN_PROGRESS" ? "active" : "default"}>
            {isUpdatingStatus ? <LoaderCircle className="animate-spin" size={14} /> : st("Mark In Progress")}
          </ActionButton>
          <ActionButton disabled={isUpdatingStatus || status === "RESOLVED"} onClick={onResolve} tone={status === "RESOLVED" ? "active" : "default"}>
            {isUpdatingStatus ? <LoaderCircle className="animate-spin" size={14} /> : st("Resolve Ticket")}
          </ActionButton>
          <ActionButton disabled={isUpdatingStatus || status === "CLOSED"} onClick={onCloseTicket} tone="danger">
            {isUpdatingStatus ? <LoaderCircle className="animate-spin" size={14} /> : st("Close Ticket")}
          </ActionButton>
          <ActionButton disabled={isUpdatingStatus || status === "OPEN"} onClick={onReopen}>
            {isUpdatingStatus ? <LoaderCircle className="animate-spin" size={14} /> : st("Reopen Ticket")}
          </ActionButton>
        </div>
      </div>
    </section>
