    ...
    return (
      <div className="flex min-h-[280px] items-center justify-center rounded-[14px] border border-dashed border-[#e5d9d0] bg-[#fcfbfa] px-6 text-center">
        <div>
          ...
          <p className="mt-3 text-[14px] font-semibold text-[#5f534b]">{vt("No sales trend is available yet.")}</p>
          <p className="mt-1 text-[12px] leading-5 text-[#8a7f76]">{vt("Revenue will appear after completed vendor orders are recorded.")}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {insights.map((insight) => (
          <div key={insight.label} className="rounded-[10px] border border-[#eee4dd] bg-[#fcfbfa] px-3 py-3">
            <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#9b8f86]">{vt(insight.label)}</p>
            <p className="mt-1 text-[16px] font-extrabold tracking-[-0.03em] text-[#201711]">{insight.value}</p>
            <p className="mt-0.5 text-[10px] text-[#887b72]">{vt(insight.detail)}</p>
          </div>
        ))}
      </div>
      <div className="rounded-[14px] border border-[#eee4dd] bg-[#fcfbfa] p-4">
        <p className="mb-5 text-xs font-semibold text-[#796659]">{vt("Revenue (NOK)")}</p>
        <div className="flex">
          ...
          <div className="min-w-0 flex-1 overflow-x-auto pb-2">
            <div style={{ minWidth: safeSeries.length * 100 }}>
              <div className="relative h-[220px] border-b border-l border-[#cdbbad]">
                ...
                <div className="absolute inset-0 grid items-end" style={{ gridTemplateColumns: `repeat(${safeSeries.length}, minmax(0, 1fr))` }} role="img" aria-label={safeSeries.map(item => `${vt(item.label)}: ${formatNok(item.value)}`).join(", ")}>
              ...
              <div className="mt-3 grid" style={{ gridTemplateColumns: `repeat(${safeSeries.length}, minmax(0, 1fr))` }}>
                {safeSeries.map((item, index) => <span key={index} className="px-1 text-center text-xs font-medium text-[#695548]">{vt(item.label)}</span>)}
              </div>
            </div>
          </div>
        </div>
        <p className="mt-2 text-center text-xs font-semibold text-[#796659]">{vt("Month")}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-2 px-1">
        ...
        <h2 className="text-[22px] font-extrabold tracking-tight text-[#18120f]">{vt("Financial Performance")}</h2>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(280px,1fr)]">
        <article className="min-w-0 rounded-[16px] border border-[#d6cbc2] bg-white p-5 shadow-[0_8px_20px_rgba(53,34,20,0.04)]">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              <h3 className="text-[20px] font-extrabold tracking-[-0.04em] text-[#18120f]">{vt("Sales performance")}</h3>
              <p className="text-[13px] font-medium text-[#8a7f76]">
                {vt(financial.chartSubtitle) || vt("Completed-order sales across the selected period.")}
              </p>
            </div>
            <span className="rounded-full border border-[#e4d7ce] bg-white px-3 py-1.5 text-[11px] font-semibold text-[#5f534b]">
              {vt(financial.filterLabel)}
            </span>
          </div>
        ...
        <div className="space-y-4">
          <article className="overflow-hidden rounded-[16px] border border-[#d6cbc2] bg-white shadow-[0_8px_20px_rgba(53,34,20,0.04)]">
            <div className="p-5">
              <p className="text-[12px] font-medium uppercase tracking-[0.03em] text-[#8c8077]">{vt("Pending Payout")}</p>
          ...
          <article className="overflow-hidden rounded-[16px] border border-[#d6cbc2] bg-white shadow-[0_8px_20px_rgba(53,34,20,0.04)]">
            <div className="p-5">
              <h3 className="text-[18px] font-medium text-[#5f534b]">{vt("Financial Breakdown")}</h3>
              <div className="mt-4 space-y-3">
                {financial.breakdown
                  .filter((item) => item.label !== "Net Earnings")
                  .map((item) => (
                    <div key={item.label} className="flex items-center justify-between gap-4">
                  <span className="text-[13px] font-medium text-[#8a7f76]">{vt(item.label)}</span>
                  <span
                    className={[
                      "text-[13px] font-bold",
                      item.tone === "negative"
            ...
            {financial.breakdown
              ...
              .map((item) => (
                ...
                >
                  <span className="text-[15px] font-bold text-[#18120f]">{vt(item.label)}</span>
