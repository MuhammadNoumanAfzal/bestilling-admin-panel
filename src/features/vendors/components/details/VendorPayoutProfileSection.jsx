...
function StatusPill({ verified, verificationStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[12px] font-bold ${tone}`}>
      ...
      {vt(verificationStatus) || (verified ? vt("Verified") : vt("Pending review"))}
    </span>
  );
}
    ...
    return (
      <section className="rounded-[18px] border border-[#ddd6cf] bg-white p-5 shadow-[0_10px_24px_rgba(55,31,13,0.05)]">
        <div className="flex items-center gap-2 text-[#221914]">
          ...
          <h2 className="text-[18px] font-bold">{vt("Payout Bank Profile")}</h2>
        </div>
        <div className="mt-4 rounded-[18px] border border-dashed border-[#ecd8cb] bg-[#fffaf6] px-4 py-5 text-[14px] leading-6 text-[#6c5d54]">{vt("This vendor has not added payout bank details yet.")}</div>
      </section>
    );
  }
      ...
      <section className="rounded-[22px] border border-[#ddd6cf] bg-white p-5 shadow-[0_14px_32px_rgba(55,31,13,0.06)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-[#221914]">
            ...
            <h2 className="text-[20px] font-bold">{vt("Payout Bank Profile")}</h2>
          </div>
          <StatusPill verified verificationStatus="Verified" />
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <DetailCell label={vt("Account Holder")} value={payoutProfile.accountHolderName} />
          <DetailCell label={vt("Bank Name")} value={payoutProfile.bankName} />
          <DetailCell label={vt("Account Number")} value={payoutProfile.accountNumber} />
        </div>
      </section>
    );
  }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[#221914]">
              ...
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#b07a5b]">{vt("Finance review")}</p>
                <h2 className="text-[22px] font-extrabold tracking-[-0.03em] text-[#221914]">{vt("Payout Bank Profile")}</h2>
              </div>
            </div>
            <p className="mt-4 max-w-[760px] text-[14px] leading-7 text-[#665850]">{vt("Review the payout bank profile carefully before any vendor payout is released. This approval should stay high priority because it directly controls finance handoff.")}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-[#e8ddd5] bg-white/90 px-3 py-2 text-[12px] font-semibold text-[#4d433d] shadow-[0_8px_20px_rgba(41,24,12,0.05)]">
              ...
              {payoutProfile.bankDetailsVerified ? vt("Ready for payout release") : vt("Waiting for admin approval")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex items-center gap-2 rounded-full bg-[#f7f2ee] px-3 py-1.5 text-[12px] font-bold text-[#9a7a68]">
            <Sparkles size={13} className="text-[#cf6e38]" />{vt("Verification controls")}</div>
        </div>
        ...
        <div className="mt-4 rounded-[22px] border border-[#f0ded2] bg-[linear-gradient(135deg,#fff8f2_0%,#fffdfa_100%)] px-4 py-4 shadow-[0_10px_22px_rgba(52,29,13,0.04)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#b07a5b]">{vt("Admin review")}</p>
              <p className="mt-2 text-[16px] font-bold text-[#221914]">{vt("Approve the bank profile before finance releases any vendor payout.")}</p>
              <p className="mt-1 text-[13px] leading-6 text-[#6f6056]">{vt("If anything does not match the vendor identity or company record, request corrections instead of approving.")}</p>
            </div>

            <div className="rounded-[18px] border border-[#efddcf] bg-white/90 p-3 shadow-[0_8px_20px_rgba(41,24,12,0.05)]">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#a08b7f]">{vt("Priority")}</p>
              <div className="mt-2 inline-flex items-center gap-2 rounded-full bg-[#fff1e8] px-3 py-1.5 text-[12px] font-bold text-[#cf6e38]">
                <BadgeCheck size={14} />
                {payoutProfile.bankDetailsVerified ? vt("Completed") : vt("Approve before payout")}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4 rounded-[20px] border border-[#ece1d7] bg-[#fcfaf8] px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[15px] font-bold text-[#211915]">{vt("Verification actions")}</p>
            <p className="mt-1 text-[13px] leading-6 text-[#665850]">
              {payoutProfile.bankDetailsVerified
                ? vt("Bank details are verified and ready for payout release.")
                : vt("Request corrections if anything does not match the vendor account or company records.")}
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <ActionButton
              ...
              icon={CheckCircle2}
              label={payoutProfile.bankDetailsVerified ? vt("Already verified") : isApproving ? vt("Approving...") : vt("Approve bank details")}
              onClick={onApprove}
            />
            {!payoutProfile.bankDetailsVerified && <ActionButton
              disabled={isApproving || isRequestingChanges}
              icon={MessageSquareWarning}
              label={isRequestingChanges ? vt("Sending...") : vt("Request changes")}
