...
function PaginationButton({ children, disabled = false, onClick, label }) {
  useVendorLanguage();
  return (
    <button
      className={[
    ...
    <section className="space-y-4">
      <div className="flex flex-col gap-3 px-1 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[22px] font-extrabold tracking-tight text-[#18120f]">{vt("Orders")}</h2>
        </div>

        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
          <label className="relative min-w-[220px] sm:w-[260px]">
            <span className="sr-only">{vt("Search orders")}</span>
            ...
            <input
              ...
              placeholder={vt("Search order, customer, event...")}
              type="search"
              value={searchTerm}
            />
          </label>
          ...
          <label className="relative min-w-[160px]">
            <span className="sr-only">{vt("Filter by delivery date")}</span>
          ...
          <label className="min-w-[160px]">
            <span className="sr-only">{vt("Filter by order status")}</span>
            <select
      ...
      <div className="overflow-hidden rounded-[16px] border border-[#cfc4bb] bg-white shadow-[0_8px_20px_rgba(53,34,20,0.04)]">
            ...
            <thead className="border-b border-[#ddd6cf] bg-[#fcfbfa]">
              <tr>
                {["Order ID", "Customer", "Event", "Guests", "Delivery date", "Status"].map((label) => (
                  <th key={vt(label)} className="px-5 py-4 text-left text-[13px] font-bold text-[#1f1711]">
                    {label}
                  </th>
                ))}
              </tr>
              ...
              {paginatedOrders.length > 0 ? (
                paginatedOrders.map((order) => (
                  <tr key={`${order.id}-${order.customer}-${order.event}`} className="border-b border-[#ece4dd] last:border-b-0">
                    ...
                    <td className="px-5 py-3.5">
                      <p className="text-[12px] font-semibold text-[#1f1711]">{vendorDate(order.deliveryDate)}</p>
                ...
                <tr>
                  <td className="px-5 py-10 text-center text-[14px] font-semibold text-[#6f645d]" colSpan={6}>
                    {hasActiveFilters ? vt("No orders match the selected filters.") : vt("No orders available yet.")}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {totalItems > PAGE_SIZE ? (
          <div className="flex flex-col gap-3 border-t border-[#ece4dd] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-[13px] text-[#6f645d]">{vt("Showing {{start}} - {{end}} of {{total}} orders", { start: (currentPage - 1) * PAGE_SIZE + 1, end: Math.min(currentPage * PAGE_SIZE, totalItems), total: totalItems })}</p>

            <div className="flex items-center gap-2">
              <PaginationButton label={vt("Previous page")} disabled={currentPage === 1} onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}>
                <ChevronLeft size={14} />
              </PaginationButton>
              ...
              <PaginationButton label={vt("Next page")} disabled={currentPage === totalPages} onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))}>
